...
Multi-image editing (up to 10 images):
...
Quick examples:
    ...
    # Ultra-tall banner
    uv run generate_image.py -p "Vertical banner ad" -f banner.png --aspect 1:8
"""
...
TEMPLATES = {
    ...
    "infographic": (
        "Clean {style} infographic about {topic}. "
        "Header: {title}. Key data: {data_points}. "
        "Use {color_scheme} color scheme, {layout} layout, "
        "legible sans-serif text, clear visual hierarchy."
    ),
}
...
def get_aspect_from_image(img) -> str:
    """Return the closest supported aspect ratio matching the image's dimensions."""
...
def get_smart_defaults(prompt: str):
    ...
    if any(word in prompt_lower for word in ["logo", "icon", "emblem"]):
        ...
        defaults["thinking"] = "high"
        defaults["add_text_guidance"] = True
    elif any(word in prompt_lower for word in ["banner", "strip", "ribbon"]):
        defaults["aspect_ratio"] = "4:1"
        defaults["add_text_guidance"] = True
    elif any(word in prompt_lower for word in ["tall banner", "skyscraper", "vertical banner"]):
        defaults["aspect_ratio"] = "1:4"
        defaults["add_text_guidance"] = True
...
def validate_prompt(prompt: str) -> list:
    ...
    if text_count > 3:
        warnings.append("Warning: You have many text elements. Limit to 1-3 for best results.")
    ...
    for realistic, stylized in style_conflicts:
        ...
        if has_realistic and has_stylized:
            warnings.append("Warning: Conflicting styles detected (e.g., photorealistic + cartoon). Pick one for best results.")
            break
    ...
    if any(word in prompt_lower for word in vague_indicators) and len(prompt.split()) < 10:
        warnings.append("Warning: Prompt is vague. Add specific details about subject, lighting, camera angle, and style.")
...
Examples:
  ...
  # Multi-image composition (up to 10)
  uv run generate_image.py -p "Group photo" -f group.png -i person1.png -i person2.png

  # Use template
  uv run generate_image.py --template photorealistic -f photo.png --template-var subject="A cat" --template-var action="sleeping"

  # Ultra-wide banner
  uv run generate_image.py -p "Website hero banner" -f hero.png --aspect 8:1
  ...
  uv run generate_image.py -p "Current Tokyo skyline at night" -f tokyo.png --search --aspect 16:9
    ...
    parser.add_argument(
        ...
        default="1K",
        help="Output resolution: 0.5K (512px), 1K (default), 2K, or 4K"
    )
    parser.add_argument(
        "--aspect", "-a",
    ...
    parser.add_argument(
        "--thinking",
        choices=["high", "minimal"],
        default="high",
        help="Thinking level: high=best quality/slower (default), minimal=fastest"
    )
    parser.add_argument(
        "--include-thoughts",
        action="store_true",
        help="Include the model's thinking process in output"
    )
    parser.add_argument(
        "--system",
        "-s",
    ...
    # Show configuration
    print(f"\nConfiguration:")
    print(f"  Model: Nano Banana 2 (gemini-3.1-flash-image-preview)")
    ...
    # Load input images if provided (up to 10 supported by Nano Banana 2)
    input_images = []
    ...
    if args.input_images:
        ...
        # Auto-detect aspect ratio from first input image if not explicitly specified
        if not args.aspect and input_images:
            args.aspect = get_aspect_from_image(input_images[0])
            print(f"Auto-detected aspect ratio from input image: {args.aspect}")
